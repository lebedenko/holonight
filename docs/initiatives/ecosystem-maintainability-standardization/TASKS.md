...
| EMS-011 | umbrella | Implement source installer, uninstaller, preflight, manifest, and docs | — | — | Done | `deef2dd` | 2026-08-12: `task --list`; `bash -n`; `scripts/install.sh --check`; complete alternate-root build/install; manifest hash/mode/symlink checks; modified-file-preserving uninstall |
| EMS-012 | umbrella | Pin published repository handoffs and run final Arch integration | EMS-001–EMS-011 | — | Done | `deef2dd` | 2026-08-12: published/pinned all handoffs; migrated 168 unowned legacy collisions to `/var/backups/holonight-source-migration-20260812-1`; real `scripts/install.sh --yes` installed 488 manifest entries; verified all hashes, revisions, permissions, symlinks, binaries, QML modules, desktop/D-Bus metadata, units, and Greeter assets; `scripts/uninstall.sh --yes` removed every recorded artifact with 0 modified paths and preserved `/etc/greetd/config.toml`; final reinstall passed 488/488 hash checks. |
