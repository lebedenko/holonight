...
Current manual request: [Batch 1 three-run checklist](BATCHES.md#batch-1-manual-runs).
The dropdown-only instructions below are historical; do not repeat accepted F06.

The reported Settings/greeter HoloNight Tab → Space → Space failure was manually
accepted on 2026-09-13; see [the acceptance record](FINDINGS.md#manual-stationary-pointer-acceptance--2026-09-13).
The instructions below are retained for reference; no repeat of that check is pending.
...
1. First repeat the reported sequence in each app: leave the pointer over where
   an unselected popup row will appear, Tab to focus the selector, Space to open,
   then Space to close without arrow keys or pointer movement. The highlight and
   committed value must remain on the original selection.
2. In Settings **Appearance**, open a font selector with enough rows to scroll.
...
3. Reopen the selector with the pointer still in the popup area. The committed
...
4. Repeat in the greeter demo session selector if it has enough rows to scroll;
...
and the printed evidence paths including `isolation.json`. F06 is closed for the reported HoloNight sequence; unrelated gates remain open. Close apps and exit Sway with Super+Shift+E.
...
The already released `06_tsrg6` kit README is immutable; apply step 1 above in
addition to its existing instructions. No runtime rebuild is required for this
clarification and test-only follow-up.
