...
Fresh kit: `/tmp/holonight-uqc201-focus-afv1kwth`.
...
The old kit and evidence are preserved. Initiative Accepted; UQC-201 In Progress.
...
```sh
python3 /tmp/holonight-uqc201-focus-afv1kwth/guided-session.py sway
```
...
Dependencies were staged into a new prefix from verified build outputs. Settings
...
`.cache/holonight-uqc201-focus-afv1kwth/{results,verification}.jsonl` and `logs/`.
...
The initial Settings startup attempt was blocked by the sandbox's private D-Bus
socket restriction; the isolated check is rerun with that capability permitted.
Final results and release status are recorded in the ledger before READY is written.
