...
def main():
    ...
    if not re.fullmatch(r"holonight-uqc(?:201-(?:final|p03)|20[78]|21[126])-[a-z0-9_]+", name):
        parser.error("expected a saved holonight-uqc201-final/p03/207/208/211/212/216-*.tar.gz kit")
    kit = Path("/tmp") / name
    if kit.exists():
    ...
    compositor = "sway" if name.startswith(("holonight-uqc201-p03-", "holonight-uqc212-", "holonight-uqc216-")) else "hyprland"
