...
- **AI:** Settings → Ollama, focus “Test connection”, press Space and wait for the
  ...
  Also activate a button that stays enabled (for example Cancel where available). For Enabled and Enable tool calling,
...
- **Settings:** inspect the track-only Switch focus outline checked and unchecked, then inspect the reported Fusion hover/spacing surfaces. Preserve Fusion's
...
- **NeoChat/Tokodon:** open the logged-out scheme dropdown and inspect background
  transparency. Leave it open briefly for geometry, palette and pixel observation. Account-dependent views are out
