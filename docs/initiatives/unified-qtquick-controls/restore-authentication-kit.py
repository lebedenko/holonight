#!/usr/bin/env python3
"""Restore a saved authentication kit to its original /tmp prefix without overwriting files."""
import argparse
import hashlib
...
def main():
    ...
    if not re.fullmatch(r"holonight-uqc205-b2_[a-z0-9_]+", name):
        parser.error("expected a saved holonight-uqc205-b2_*.tar.gz kit")
    kit = Path("/tmp") / name
    if kit.exists():
    ...
    print(f"From a fresh tux VT login: python3 {kit}/guided-session.py hyprland")
