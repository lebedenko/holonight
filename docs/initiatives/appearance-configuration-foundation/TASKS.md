...
| ACF-002 | umbrella | Classify every field; select canonical document format/path, shared schema owner, single-writer/notification contract, clean-break migration policy, and metric taxonomy | ACF-001 | This initiative | Planned | — | — |
| ACF-003 | Shared configuration owner (TBD) | Implement and publish canonical neutral appearance schema, defaults, validation, path resolution, atomic serialization, diagnostics, and any accepted one-time migration | ACF-002 | Pending | Planned | — | — |
...
| ACF-007 | umbrella | Pin published revisions and verify the clean break, migration/rollback, configuration security boundary, exact contracts, and complete repository test order | ACF-003–ACF-006 | — | Planned | — | — |
